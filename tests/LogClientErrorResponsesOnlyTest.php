...
use Mtownsend\RequestResponseLogger\Support\Logging\LogClientErrorsOnly;
use Orchestra\Testbench\TestCase;

class LogClientErrorResponsesOnlyTest extends TestCase
{
    use RefreshDatabase;
    ...
    protected function defineEnvironment($app)
    {
        ...
        $app['config']->set('log-requests-and-responses', [
            ...
            'should_log_handler' => LogClientErrorsOnly::class,
        ]);
    }
     ...
     * @test
     */
    public function logClientErrorResponsesOnly()
    {
        $request = $this->post('/test/200'); // success
        $request = $this->post('/test/401'); // error
        $request = $this->post('/test/201'); // success
